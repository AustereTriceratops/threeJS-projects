...
class Simulator
{
	...
	static updateCameraPosition()
	{
		...
		{
			var cameraYDirection = Simulator.cameraY.clone().multiplyScalar(0.05);
			Simulator.cameraPos.add(cameraYDirection);
		}
		if (Simulator.keyTracker["a"])
		{
			var cameraXDirection = Simulator.cameraX.clone().multiplyScalar(0.05);
		...
		{
			var cameraZDirection = Simulator.cameraZ.clone().multiplyScalar(0.05);
